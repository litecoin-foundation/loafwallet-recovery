(function() {
    ...
    function init() {
        ...
        DOM.balanceToggle.on("click", toggleBalance);
    ...
    function TableRow(index, isLast) {
        ...
        function calculateValues() {
            setTimeout(function() {
                ...
                // query Litecoin Space for balance
                // convert from sats to whole ltc + append LTC
                var baseURL = "https://litecoinspace.org/api/address/" + address;
                var balance;
                $.ajax({
                    url: baseURL,
                    success: function(data) {
                        var fundedSum = data.chain_stats.funded_txo_sum;
                        var spentSum = data.chain_stats.spent_txo_sum;
                        var balanceSats = fundedSum - spentSum;
                        var balanceLTC = balanceSats / 100000000;
                        balance = balanceLTC.toString() + " LTC";
                    },
                    async: false,
                });
                addAddressToList(indexText, address, balance, privkey);
                if (isLast) {
                    hidePending();
                    updateCsv();
                }
    ...
    //LOSHAN: adds data to the table
    function addAddressToList(indexText, address, balance, privkey) {
